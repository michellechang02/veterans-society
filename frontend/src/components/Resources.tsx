...
const Resources: React.FC = () => {
    ...
    if ('geolocation' in navigator) {
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          try {
            const veteranResources = await OverpassService.getVeteranResourcesByLocation(
              location.lat,
              location.lon
            );
            ...
            if (veteranResources.length > 0) {
              ...
              toast({
                ...
                description: 'No veteran resources found in your immediate area. Try again later or contact VA support for assistance.',
                status: 'info',
                duration: 5000,
                isClosable: true,
              });
            ...
            toast({
              title: 'Error',
              description: 'Failed to fetch veteran resources. Please try again later.',
              status: 'error',
              duration: 5000,
              isClosable: true,
            });
        ...
        (error) => {
          ...
          setError('Location access denied. Please enable location services to find nearby veteran resources.');
          setIsLoading(false);
          toast({
            title: 'Location Access Denied',
            description: 'Please enable location services to find nearby veteran resources.',
            status: 'error',
            duration: 5000,
            isClosable: true,
          });
  ...
  return (
    <Flex h="calc(100vh - 64px)" // Adjust based on your navbar height
